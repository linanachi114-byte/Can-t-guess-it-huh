  ...
  const content = await askDeepSeek([
    {
      role: "system",
      content:
        "你要为中文猜词游戏批量生成分层线索。必须优先理解词库名代表的语境：同一个词在不同词库里含义可能完全不同，例如词库是“杀戮尖塔”时，“悔恨”应理解为游戏中的诅咒卡，而不是普通情绪。每个目标生成 3 条线索，按从宽到窄排列。每条线索只能包含一个事实，不能把多个特征合并在同一句里。第 1 条只说大领域或大类别，第 2 条只说一个泛属性，第 3 条给一个中等强度特征。不要直接写出答案，不要包含答案完整词语，不要使用唯一定位式描述。避免“最大、唯一、标志、代表作、具体节日、作者、歌手、画家”等一眼锁定的信息。每条 4 到 12 个中文字符。每个目标输出一行，格式必须是：目标词<TAB>线索1<TAB>线索2<TAB>线索3。不要编号，不要解释。"
    },
    {
      role: "user",
...
function fallbackClues(category) {
  return [`来自${category}词库`, "有明确辨识特征", "常见于特定语境"];
}
