(function () {
  ...
  function installAuthGate() {
    ...
    style.textContent = `
      ...
      .nanachi-auth-head p{margin:0;color:#766958;font-size:14px;line-height:1.5}
      .nanachi-auth-tabs{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:14px;padding:5px;border-radius:17px;background:#efe2cf}
      .nanachi-auth-tabs button{border:0;border-radius:13px;min-height:42px;background:transparent;color:#796b59;font:inherit;font-weight:900;cursor:pointer}
      .nanachi-auth-tabs button.active{background:#fffaf2;color:#2d2318;box-shadow:0 6px 18px rgba(71,45,19,.12)}
      .nanachi-auth-form{display:grid;gap:10px}
      .nanachi-auth-form label{display:grid;gap:6px;color:#6c604f;font-size:13px;font-weight:800}
      .nanachi-auth-form input{width:100%;box-sizing:border-box;border:1px solid #decfb9;border-radius:15px;background:#fffdf8;color:#2d2318;font:inherit;font-weight:800;padding:13px 14px;outline:none}
      .nanachi-auth-form input::-ms-reveal,.nanachi-auth-form input::-ms-clear{display:none}
      .nanachi-auth-form input:focus{border-color:#df9a39;box-shadow:0 0 0 4px rgba(223,154,57,.16)}
      .nanachi-auth-password-wrap{position:relative}
      .nanachi-auth-password-wrap input{padding-right:46px}
      .nanachi-auth-password-toggle{position:absolute;right:8px;top:50%;width:32px;height:32px;transform:translateY(-50%);border:0;border-radius:999px;display:grid;place-items:center;background:transparent;color:#806f5b;cursor:pointer}
      .nanachi-auth-password-toggle:hover{background:rgba(82,54,25,.08)}
      .nanachi-auth-password-toggle svg{width:18px;height:18px}
      .nanachi-auth-error{min-height:20px;color:#b54c42;font-size:13px;font-weight:800}
      .nanachi-auth-submit{border:0;border-radius:17px;min-height:50px;background:#db8f2f;color:#fffaf2;font:inherit;font-size:16px;font-weight:1000;cursor:pointer}
      ...
      @media(max-width:560px),(max-height:760px){
        ...
        .nanachi-auth-gate.profile-auth{align-items:flex-start;padding:8px;overflow-y:auto}.nanachi-auth-gate.profile-auth .nanachi-auth-panel{width:100%;max-height:calc(100dvh - 16px);margin:0;padding:13px 14px;border-radius:16px}.nanachi-auth-gate.profile-auth .nanachi-auth-close{left:8px;right:auto;top:8px}.nanachi-auth-gate.profile-auth .nanachi-auth-head{gap:6px;margin:2px 0 8px}.nanachi-auth-gate.profile-auth .nanachi-auth-mark{width:46px;height:46px}
      }
    `;
    ...
    gate.innerHTML = `
      <section class="nanachi-auth-panel" role="dialog" aria-modal="true" aria-labelledby="nanachi-auth-title">
        ...
        <div class="nanachi-auth-head">
          ...
          <div>
            <h2 id="nanachi-auth-title">登录猜不到吧</h2>
            <p>统一账号登录后，猜题记录会保存到游戏厅。</p>
          </div>
        </div>
    ...
    function setMode(mode) {
      ...
      title.textContent = mode === "register" ? "注册猜不到吧" : "登录猜不到吧";
  ...
  function launcherUrl() {
    ...
    const hostname = window.location.hostname;
    const isLocal = hostname === "localhost" || hostname === "127.0.0.1" || hostname === "::1";
    return isLocal ? "http://127.0.0.1:5175" : "https://lijiaqi.me";
  }
  ...
  function installGameCenterBackGuard() {
    ...
    const style = document.createElement("style");
    style.textContent = `
      .nanachi-exit-overlay{position:fixed;inset:0;z-index:9999;display:none;place-items:center;background:rgba(45,31,19,.48);backdrop-filter:blur(10px);padding:20px}
      .nanachi-exit-overlay.show{display:grid}
      .nanachi-exit-dialog{width:min(360px,100%);border-radius:24px;border:1px solid rgba(132,93,45,.18);background:#fff8ec;box-shadow:0 24px 70px rgba(82,54,25,.3);padding:20px;color:#2d2318;font-family:inherit}
      .nanachi-exit-dialog h2{margin:0 0 8px;font-size:22px;line-height:1.2}
      .nanachi-exit-dialog p{margin:0 0 18px;color:#716557;line-height:1.55;font-size:14px}
      .nanachi-exit-actions{display:grid;grid-template-columns:1fr 1fr;gap:10px}
      .nanachi-exit-actions button{border:0;border-radius:16px;min-height:46px;font:inherit;font-weight:900;cursor:pointer}
      .nanachi-exit-cancel{background:#eee4d4;color:#3b3024}
      .nanachi-exit-ok{background:#db8f2f;color:#fffaf2}
    `;
    document.head.appendChild(style);
    ...
    function hasProjectInternalState(state) {
      return Boolean(state?.yichuiInternal || state?.innerScreen);
    }
